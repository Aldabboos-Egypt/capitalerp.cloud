...
class subscription_subscription(models.Model):
    ...
    def get_invoiced_count(self):
        for rec in self:
            ...
            if rec.so_origin and self.so_origin.invoice_ids:
                so_inv =  rec.so_origin.invoice_ids.filtered(lambda inv: inv.payment_state == 'paid')
                rec.invoice_count = len(rec.invoice_ids) + len(so_inv) if so_inv else len(rec.invoice_ids)
            if rec.invoice_count == rec.num_billing_cycle:
    ...
    def action_view_invoice(self):
        ...
        if self.so_origin and self.so_origin.invoice_ids:
            invoices += self.so_origin.invoice_ids.filtered(lambda inv: inv.payment_state == 'paid')
        if len(invoices) > 1:
