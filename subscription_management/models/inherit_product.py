    ...
    def on_change_activate_subscription(self):
        for current_rec in self:
            if current_rec.activate_subscription:
                ...
                current_rec.detailed_type = 'service'
            else:
...
class ProductTemplate(models.Model):
        ...
        if template_id.activate_subscription:
            for product in template_id.product_variant_ids:
                product.write({'activate_subscription': True, 'subscription_plan_id':
                              template_id.subscription_plan_id.id if template_id.subscription_plan_id else False, 'detailed_type': 'service'})
        return template_id
    ...
    def activate_subscription_check(self):
        if self.activate_subscription and self.detailed_type != 'service':
    ...
    def _get_combination_info(self, combination=False, product_id=False, add_qty=1, pricelist=False, parent_combination=False, *args, **kwargs):
        ...
        pricelist = pricelist if pricelist else self.env.user.company_id.partner_id.property_product_pricelist
        if not product_id:
            ...
            if product.activate_subscription:
                ...
                if override:
                    ...
                    list_price = product_template.currency_id._convert(
                        ...
                        product.price_extra, pricelist.currency_id, product_template._get_current_company(
                            pricelist=pricelist),
                        fields.Date.today()
                    )
                    price = list_price
                ...
                if product.activate_subscription:
                    values = {
                        'price_unit':  product.product_tmpl_id.currency_id._convert(
                            product.subscription_plan_id.plan_amount +
                            product.price_extra, order.pricelist_id.currency_id, product.product_tmpl_id._get_current_company(
